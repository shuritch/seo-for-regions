...
Возможно это самый популярный сейчас подход по региональному продвижению в СНГ. В основном это из-за популярности у нас поисковой системы от **Yandex**, которая предоставляет функционал для определения точного региона домена прямо из своей панели **Yandex webmaster**.

Домены с определенным регионом всплывают _по гео-зависимым запросом_. Однако есть и **Минус**, в похожем функционале от Google на выбор есть только регион "Россия", из-за чего подход показывает себя хуже.
...
> [!TIP]
> Лучше всего это работает когда ваш разработчик настроил wildcard домен третьего уровня с перенаправлением на один сервер `*.website.ru`, где название поддомена обрабатывается как переменная для отбора нужных ключей.
>
...
На проекте уже использовался доменный метод регионального продвижения, так что оставалось лишь ввести второй способ и настроить правильную индексацию - чтобы не нарушить уникальности контента и избежать санкций от поисковых систем.
