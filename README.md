...
>
> В своей практике, я не раз сталкивался с не правильным способом реализации - когда доменную региональность реализовывали с помощью нескольких веб-серверов, каждый под свой поддомен. Такой подход чаще всего приводит к неоправданным **расходом** и **увеличенному TTM**.
...
Чаще всего эта версия попадается на интернет-барахолках, где пользователи сами выставляют товар на продажу. Так как нет четкого представления регионов веб-сайта, он приписывается каждому товару по отдельности самим пользователем, а система добавляет его в мета-теги и текста, а иногда и в URL. Примерная структура:
...
Однако, по какой-то неведомой мне причине, я не нашел ни одного упоминания данного способа. Впрочем, никто не был против данного эксперимента, и **тщательно** все взвесив - я решил экспериментировать !
...
Помимо этого я внедрил _Умные ссылки_, _HTML Карту сайта_ и обновил _sitemap.xml_, а также настроил _городской выбор_ на один домен, но об этой рутине когда нибудь в другой раз..

### Настраиваем правильную индексацию

С моей точки зрения, правильная индексация для данного подхода наступит при:

- Закрытых от индексации Яндексом вложенных регионов
- Закрытых от индексации другими Поисковыми системами поддоменов
- И не запутавшимся во всем этом разработчике
